...
- Проверено: повторная индексация учеников не обновляет защищённые tenant/Telegram-поля, самостоятельно чинит отсутствующие строки и запускается из API и фонового worker.
- Полный набор Node.js: 23/23 теста пройдено.
- `index.html`, `docs/index.html` и `docs/404.html` идентичны; `health.json` содержит `8.3.1-beta.1`.
