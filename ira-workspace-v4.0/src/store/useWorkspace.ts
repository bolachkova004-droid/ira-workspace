...
import type {ContentItem,Lead,LeadStatus,Lesson,Material,Student,Task,Touchpoint} from '../types'
...
function readAny<T>(keys:string[],fallback:T):T{
  for(const key of keys){
    try{const value=localStorage.getItem(key);if(value!==null)return JSON.parse(value) as T}catch{/* use next key */}
  }
  return fallback
}
...
export function useWorkspace(){
  const [contentItems,setContentItems]=useState<ContentItem[]>(()=>readAny(['ira.v4.content','ira.v3.content'],contentSeed))
  const [touchpoints,setTouchpoints]=useState<Touchpoint[]>(()=>readAny(['ira.v4.touchpoints','ira.v3.touchpoints'],touchpointsSeed))
  const [students,setStudents]=useState<Student[]>(()=>readAny(['ira.v4.students','ira.v2.students'],studentsSeed))
  const [leads,setLeads]=useState<Lead[]>(()=>readAny(['ira.v4.leads','ira.v2.leads'],leadsSeed))
  const [lessons,setLessons]=useState<Lesson[]>(()=>readAny(['ira.v4.lessons','ira.v2.lessons'],lessonsSeed))
  const [tasks,setTasks]=useState<Task[]>(()=>readAny(['ira.v4.tasks','ira.v2.tasks'],tasksSeed))
  const [materials,setMaterials]=useState<Material[]>(()=>readAny(['ira.v4.materials','ira.v2.materials'],materialsSeed))
  const [dark,setDark]=useState(initialDark)

  useEffect(()=>localStorage.setItem('ira.v4.content',JSON.stringify(contentItems)),[contentItems])
  useEffect(()=>localStorage.setItem('ira.v4.touchpoints',JSON.stringify(touchpoints)),[touchpoints])
  useEffect(()=>localStorage.setItem('ira.v4.students',JSON.stringify(students)),[students])
  useEffect(()=>localStorage.setItem('ira.v4.leads',JSON.stringify(leads)),[leads])
  useEffect(()=>localStorage.setItem('ira.v4.lessons',JSON.stringify(lessons)),[lessons])
  useEffect(()=>localStorage.setItem('ira.v4.tasks',JSON.stringify(tasks)),[tasks])
  useEffect(()=>localStorage.setItem('ira.v4.materials',JSON.stringify(materials)),[materials])
  useEffect(()=>{localStorage.setItem('ira.v4.dark',JSON.stringify(dark));document.documentElement.dataset.theme=dark?'dark':'light';applyTelegramChrome(dark)},[dark])
  ...
  const insights=useMemo(()=>{
    ...
    if(debtStudent)result.push(`${debtStudent.name.split(' ')[0]}: нужно напомнить об оплате ${Math.abs(debtStudent.balance).toLocaleString('ru-RU')} ₽.`)
    const freshLead=leads.find(lead=>lead.status==='Новая')
    if(freshLead)result.push(`Новая заявка от ${freshLead.name} из ${freshLead.source}.`)
    return result
  },[students,leads])

  return {
    students,leads,lessons,tasks,materials,touchpoints,contentItems,dark,setDark,stats,insights,
    addStudent:(value:Omit<Student,'id'>)=>setStudents(items=>[{...value,id:crypto.randomUUID()},...items]),
    ...
    addLead:(value:Omit<Lead,'id'|'createdAt'|'status'>)=>setLeads(items=>[{...value,id:crypto.randomUUID(),createdAt:'Только что',status:'Новая'},...items]),
    ...
    addLesson:(value:Omit<Lesson,'id'>)=>setLessons(items=>[...items,{...value,id:crypto.randomUUID()}]),
    ...
    addTask:(title:string)=>setTasks(items=>[{id:crypto.randomUUID(),title,due:'Сегодня',done:false,category:'Другое'},...items]),
    ...
    addContentItem:(value:Omit<ContentItem,'id'>)=>setContentItems(items=>[{...value,id:crypto.randomUUID()},...items]),
    ...
    addTouchpoint:(value:Omit<Touchpoint,'id'>)=>setTouchpoints(items=>[{...value,id:crypto.randomUUID()},...items]),
    ...
    addMaterial:(value:Omit<Material,'id'>)=>setMaterials(items=>[{...value,id:crypto.randomUUID()},...items]),
