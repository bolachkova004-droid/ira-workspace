...
const createId=()=>{
  try{return crypto.randomUUID()}catch{return `ira-${Date.now()}-${Math.random().toString(36).slice(2)}`}
}

const writeStorage=(key:string,value:unknown)=>{
  try{localStorage.setItem(key,JSON.stringify(value))}catch(error){console.warn(`[Ira Workspace] Could not save ${key}`,error)}
}
...
export function useWorkspace(){
  ...
  return {
    ...
    addStudent:(value:Omit<Student,'id'>)=>setStudents(items=>[{...value,id:createId()},...items]),
